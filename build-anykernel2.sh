...
# Added: Ability to now change ToolChains on the Go Without Restarting                    #
# Changed: De-Nebulaized Build Script                                                     #
#                                                                                         #
###########################################################################################
###########################################################################
###                                                                     ###
...
function check_filesize() {
	...
	maxsize=18
	cd $ZIP_MOVE
	file=${KNAME}_"$REV""_MR_""$VARIANT""_""$KVER"".zip"
...
function make_zip {
		...
		cd $REPACK_DIR
		zip -r9 ${KNAME}_"$REV"_MR_"$VARIANT"_"$KVER".zip *
		mv ${KNAME}_"$REV"_MR_"$VARIANT"_"$KVER".zip $ZIP_MOVE
		rm -rf ${KNAME}_"$REV"_MR_"$VARIANT"_"$KVER".zip
		cd $KERNEL_DIR
}
...
function finished_build {
	...
	--infobox "${KNAME}_'$REV'_MR_'$VARIANT'_'$KVER'.zip \n\
	Created Successfully..\n\
...
echo -e "${green}"
echo "--------------------------------------------------------"
echo "${KNAME}'$REV'_MR_'$VARIANT'_'$KVER'-signed.zip"
echo "Created Successfully.."
